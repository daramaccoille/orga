...
export class AdvancedTechnicalAnalysis {
  ...
  calculateStochastic(prices: number[], period = 14, kPeriod = 3, dPeriod = 3): { k: number; d: number } {
  ...
  calculateIchimoku(highs: number[], lows: number[], periodTenkan = 9, periodKijun = 26, periodSenkouB = 52, periodChikou = 26): { tenkan: number; kijun: number; senkouA: number; senkouB: number; chikou: number } {
...
// add for SMA strategy
  calculateSMAs(prices: number[], shortPeriod = 20, longPeriod = 50): { short: number; long: number } {
  ...
  calculateTimeframedIndicators(prices: TimeframedData): AdvancedIndicators['timeframes'] {
